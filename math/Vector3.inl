...
namespace gameutil 
{
	...
	{
		return createVector<T>(x + v.x, y + v.y, z + v.z);
	}
	template <typename T>
	inline const Vector3<T> Vector3<T>::operator +  (const T& s) const {
		return createVector<T>(x + s, y + s, z + s);
	}
	template <typename T>
	...
	// Create a new vector 
	template <typename T>
	inline const Vector3<T> createVector(const T& x, const T& y, const T& z) {
		Vector3<T> a;
		a.x = x;
		a.y = y;
		a.z = z;
	...
	template <typename T>
	inline const Vector3<T> createVector(const T& s) {
		Vector3<T> a;
		a.x = s;
		a.y = s;
		a.z = s;
